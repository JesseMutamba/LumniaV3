...
function ScopedWorkspace({org,principal,onExpired,onSignOut,user,workspaceName}) {
  ...
  const publicationRequest=useMemo(()=>principal==='client'?createPublicationRequest({org,onExpired}):undefined,[principal,org])
...
export default function GeneralWorkspace({orgs=[]}) {
  ...
  return <div><div className="fr-financial-context"><label>Client <select value={org} onChange={e=>selectOrg(e.target.value)} aria-label="Analysis client"><option value="" disabled>Select client</option>{orgs.map(o=><option key={o.id} value={o.id}>{o.name||o.id}</option>)}</select></label><small>Prepare, understand, analyze, and save dashboards for this client.</small></div>{org?<ScopedWorkspace key={'author:'+org} principal="author" org={org}/>:<p>Create a client in Reports & operations to begin.</p>}</div>
}
