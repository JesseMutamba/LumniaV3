...
export function analyzeWorkbooks(planBook:InspectedWorkbook,actualBook?:InspectedWorkbook,title='Client financial review'):Review{
 ...
 if(actualBook){
  ...
  if(actualCandidates.length!==1)issues.push({id:'actual-map',severity:'warning',title:'Actual production/cost table needs review',detail:'A single monthly actuals table containing total costs, FFB and CPO could not be identified. Quarter comparisons are left unavailable.',refs:[]});
  else {
   ...
   const allSeries=[...Object.values(actualValues),...Object.values(monthly).map(v=>v?.points||[])];const quarterMonths=[1,2,3].filter(m=>allSeries.every(series=>series.some(p=>p.month===m)));
   for(const id of ['opex','ffb','cpo'] as const){const a=actualValues[id].filter(p=>p.month<=3),p=monthly[id]?.points.filter(p=>p.month<=3)||[],common=new Set(quarterMonths),selectedActual=a.filter(p=>common.has(p.month)),selectedPlan=p.filter(p=>common.has(p.month)),unit=id==='opex'?(unitProven?'USD':'UNVERIFIED'):'t';
 ...
 const books=actualBook?[planBook,actualBook]:[planBook];
